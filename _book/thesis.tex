...
\begin{figure}
\includegraphics[width=0.9\linewidth]{thesis_files/figure-latex/height-model-1} \caption[Tree height predictive model]{ootnotesize{Predictive model for tree height from measured DBH. A third order polynomial regression was used in order to account for the variation between species. (Adjusted R-squared = 0.72, P = <2.2e-16)}}\label{fig:height-model}
\end{figure}
\begin{figure}
\includegraphics[width=0.9\linewidth]{thesis_files/figure-latex/width-model-g-1} \caption[Crown width predictive model]{test}\label{fig:width-model-g}
\end{figure}
\hypertarget{point-method-1}{%
\section{Point Method}\label{point-method-1}}

With the selection of CNH trees and 2021 NDVI data, a statistical analysis of the point value method shows that there is a statistically significant difference in the average NDVI values between health categorizations of ``Fair'' and ``Good'' (ANOVA, \(F_{2, 109} = 3.892\), \(P = 0.023\), TukeyHSD). There is a general positive correlation between NDVI and health category, specifically Fair and Good (Figure \ref{fig:point-all}). When split by species, the relationship between health and NDVI is more apparent in the two maple species, but still holds in the ``fair'' and ``good'' categories in the two coniferous species (Figure \ref{fig:point-species}).
\begin{figure}
\includegraphics[width=0.9\linewidth]{thesis_files/figure-latex/point-all-1} \caption[Point Method NDVI and Health Rating]{NDVI extracted for CNH trees using the point method compared to CNH tree health categorization. There is a statistically significant difference between 'Fair' and 'Good' (ANOVA, $F_{2, 109} = 3.892$, $P = 0.023$, TukeyHSD)}\label{fig:point-all}
\end{figure}
\includegraphics[width=0.9\linewidth]{thesis_files/figure-latex/point-species-1}

\hypertarget{radius-method-1}{%
\section{Radius Method}\label{radius-method-1}}

With the radius method of pixel selection and crown delineation, there is a statistically significant difference in the mean NDVI values for health categories (ANOVA, \(F_{2,109}=4.923\), \(P = 0.0089\); TukeyHSD: good-poor \(P = 0.033\), good-fair \(P = 0.014\)). Similar to the point method, there is a larger difference in average NDVI between the ``fair'' and ``good'' categories than ``poor'' and ``fair'', and with the radius method, this is now statistically significant (Figure \ref{fig:radius-all}).
\begin{figure}
\includegraphics{thesis_files/figure-latex/radius-all-1} \caption[NDVI and health rating for Radius method]{Boxplot of average NDVI for each CNH tree health categorization. There is a statistically significant difference between good and poor, as well as good and fair. (ANOVA, $F_{2,109}=4.923$, $P = 0.0089$; TukeyHSD: good-poor $P = 0.033$, good-fair $P = 0.014$)}\label{fig:radius-all}
\end{figure}
\begin{figure}
\includegraphics[width=0.9\linewidth]{thesis_files/figure-latex/radius-species-1} \caption[Radius method mean NDVI and health condition by species]{Boxplot of average NDVI from radius method and health condition, by species. The relationship berween NDVI and health category is strongest in the maples, but still somewhat aparent in the conifers.}\label{fig:radius-species}
\end{figure}
\hypertarget{lidar-method-1}{%
\section{LiDAR Method}\label{lidar-method-1}}

There is a statistically significant difference in NDVI between health categories for the LiDAR method, specifically between good and fair (ANOVA, \(F_{2,101} = 5.405\), \(P = 0.00589\), TukeyHSD)

\includegraphics{thesis_files/figure-latex/unnamed-chunk-14-1.pdf}
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cnh\_lidar }\SpecialCharTok{\%\textgreater{}\%}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ health\_rat, }\AttributeTok{y =}\NormalTok{ sample\_ndvi)) }\SpecialCharTok{+} \FunctionTok{scale\_color\_manual}\NormalTok{(}\AttributeTok{values =}\NormalTok{ species\_pal) }\SpecialCharTok{+}
    \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{color =}\NormalTok{ species), }\AttributeTok{alpha =} \FloatTok{0.2}\NormalTok{, }\AttributeTok{size =} \DecValTok{3}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{facet\_wrap}\NormalTok{(}\SpecialCharTok{\textasciitilde{}}\NormalTok{species) }\SpecialCharTok{+} \FunctionTok{theme\_minimal}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"CNH average NDVI by health condition and species"}\NormalTok{,}
    \AttributeTok{x =} \StringTok{"Health Rating"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Mean NDVI"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
There is a statistically significant difference in NDVI values for the three different pixel selection methods (ANOVA, \(F_{2, 325}=517.8\), \(P = <2e-16\), TukeyHSD: radius-point \(P = 0.00\), lidar-point \(P = 0.00\)).

\includegraphics{thesis_files/figure-latex/methods-all-species-1.pdf}
\includegraphics{thesis_files/figure-latex/methods-species-1.pdf}
