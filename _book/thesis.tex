...
The NDVI calculations were done using Python in PyCharm. Besides minor alterations, the processing code came from Planet's instructions on calculating NDVI from 4-band PlanetScope data (Planet, n.d.). NDVI is calculated from the red and NIR satellite bands (Equation \eqref{eq:ndvi-eq})
\begin{equation}
...
\end{figure}
These three processes were repeated for random samples of 100 park trees and 100 street trees, though due to restrictions in file size for LiDAR processing, the sampling had to be constrained to an area of East Portland (Figures \ref{fig:clip-extent}, \ref{fig:random-sample-park}, \ref{fig:random-sample-street}).
\begin{figure}

{\centering \includegraphics[width=1\linewidth]{figure/pdx_extent} 

}

\caption{Geographic extent for random sampling}\label{fig:clip-extent}
\end{figure}
\begin{figure}

{\centering \includegraphics[width=1\linewidth]{figure/random_park_trees} 

}

\caption{Random sample of 100 park trees}\label{fig:random-sample-park}
\end{figure}
\begin{figure}

{\centering \includegraphics[width=1\linewidth]{figure/random_street_trees} 

}

\caption{Random sample of 100 street trees}\label{fig:random-sample-street}
\end{figure}
...
\hypertarget{cnh-tree-data}{%
\section{CNH Tree Data}\label{cnh-tree-data}}
...
The tree height model has an adjusted R-squared value of 0.78, and a p-value of \textless{} 2.2e-16. The crown width model has an adjusted R-squared value of 0.72, and a p-value of \textless{} 2.2e-16.

HEY AARON AND ANNA - WHAT AND HOW DO I WRITE ABOUT THIS??

\includegraphics{thesis_files/figure-latex/height-model-1.pdf}
\includegraphics{thesis_files/figure-latex/width-model-1.pdf}

\hypertarget{ndvi-calculation-1}{%
\section{NDVI Calculation}\label{ndvi-calculation-1}}

\hypertarget{point-method-1}{%
\section{Point Method}\label{point-method-1}}
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cnh\_point }\OtherTok{\textless{}{-}} \FunctionTok{read\_csv}\NormalTok{(}\StringTok{"data/cnh\_point.csv"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{health\_rat =} \FunctionTok{fct\_relevel}\NormalTok{(health\_rat, }\AttributeTok{levels =} \FunctionTok{c}\NormalTok{(}\StringTok{"poor"}\NormalTok{, }\StringTok{"fair"}\NormalTok{, }\StringTok{"good"}\NormalTok{))) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{filter}\NormalTok{(}\SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(health\_rat))}

\NormalTok{cnh\_point }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ health\_rat, }\AttributeTok{y =}\NormalTok{ sample\_ndvi))}\SpecialCharTok{+}
  \FunctionTok{geom\_boxplot}\NormalTok{()}\SpecialCharTok{+}
  \CommentTok{\#geom\_point(aes(color = species))+}
  \FunctionTok{theme\_minimal}\NormalTok{()}\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Average NDVI measurement for CNH trees, by assigned health rating"}\NormalTok{, }\AttributeTok{x =} \StringTok{"Health Rating"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Mean NDVI"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{cnh\_point }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{theme\_minimal}\NormalTok{()}\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"CNH average NDVI by health condition and species"}\NormalTok{, }\AttributeTok{x =} \StringTok{"Health Rating"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Mean NDVI"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
\includegraphics{thesis_files/figure-latex/unnamed-chunk-7-2.pdf}

\hypertarget{radius-method-1}{%
\section{Radius Method}\label{radius-method-1}}
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cnh\_radius }\OtherTok{\textless{}{-}} \FunctionTok{read\_csv}\NormalTok{(}\StringTok{"data/cnh\_radius.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cnh\_lidar }\OtherTok{\textless{}{-}} \FunctionTok{read\_csv}\NormalTok{(}\StringTok{"data/cnh\_lidar.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{street\_counts}\OtherTok{\textless{}{-}}\FunctionTok{get\_pdxTrees\_streets}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{select}\NormalTok{(Species, Scientific, year) }\SpecialCharTok{\%\textgreater{}\%}
...
\NormalTok{park\_counts }\OtherTok{\textless{}{-}}\FunctionTok{get\_pdxTrees\_parks}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{filter}\NormalTok{(Species }\SpecialCharTok{\%in\%} \FunctionTok{c}\NormalTok{(}\StringTok{"ACMA"}\NormalTok{, }\StringTok{"ACPL"}\NormalTok{, }\StringTok{"PSME"}\NormalTok{, }\StringTok{"THPL"}\NormalTok{)) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{year =} \FunctionTok{year}\NormalTok{(Inventory\_Date)) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{select}\NormalTok{(Species, Scientific\_Name, year) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{add\_count}\NormalTok{(Species, year) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{distinct}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{group\_by}\NormalTok{(Species) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{Total =} \FunctionTok{sum}\NormalTok{(n)) }\SpecialCharTok{\%\textgreater{}\%}
...
\CommentTok{\# delineate tree crowns}
\DocumentationTok{\#\# inputs: my tree points, LiDAR data}
\NormalTok{crownsPoly }\OtherTok{\textless{}{-}} \FunctionTok{mcws}\NormalTok{(}\AttributeTok{treetops =}\NormalTok{ all\_ttops, }\AttributeTok{CHM =}\NormalTok{ EML\_CHM, }\AttributeTok{format =} \StringTok{"polygons"}\NormalTok{, }\AttributeTok{minHeight =} \DecValTok{20}\NormalTok{, }\AttributeTok{verbose =} \ConstantTok{TRUE}\NormalTok{)}

\CommentTok{\# export file}
\FunctionTok{writeOGR}\NormalTok{(crownsPoly, }\StringTok{"\textasciitilde{}/Desktop/thesis\_data/lidar\_crown"}\NormalTok{, }\StringTok{"canopy\_delin\_polygons\_poly"}\NormalTok{, }\AttributeTok{driver =} \StringTok{"ESRI Shapefile"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
\backmatter
