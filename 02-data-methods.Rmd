...
street_counts<-get_pdxTrees_streets() %>%
  ...
  select(Species, Scientific, year) %>%
...
park_counts <-get_pdxTrees_parks() %>%
  filter(Species %in% c("ACMA", "ACPL", "PSME", "THPL")) %>%
  mutate(year = year(Inventory_Date)) %>%
  select(Species, Scientific_Name, year) %>%
  add_count(Species, year) %>%
  distinct() %>%
  group_by(Species) %>%
  mutate(Total = sum(n)) %>%
...
The NDVI calculations were done using Python in PyCharm. Besides minor alterations, the processing code came from Planet's instructions on calculating NDVI from 4-band PlanetScope data [@planetndvi]. NDVI is calculated from the red and NIR satellite bands (Equation \@ref(eq:ndvi-eq))
...
```{r clip-extent, out.width = '100%', warnings=FALSE, messages=FALSE, fig.show="hold", fig.cap="Geographic extent for random sampling", fig.align='center', echo = FALSE}
knitr::include_graphics("figure/pdx_extent.png")
```

```{r random-sample-park, out.width = '100%', warnings=FALSE, messages=FALSE, fig.show="hold", fig.cap="Random sample of 100 park trees", fig.align='center', echo = FALSE}
knitr::include_graphics("figure/random_park_trees.png")
```

```{r random-sample-street, out.width = '100%', warnings=FALSE, messages=FALSE, fig.show="hold", fig.cap="Random sample of 100 street trees", fig.align='center', echo = FALSE}
knitr::include_graphics("figure/random_street_trees.png")
```
