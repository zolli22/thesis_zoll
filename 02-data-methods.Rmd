...
**good**, **fair**, **poor**, or dead (Table \@ref(tab:var-type-table)).
...
<!-- Needs more detail. You should write out the equations of the model and the statistical results. And this was the same model used to identify trees to sample last summer, correct? (to identify trees that were >25’ height, based on known DBH) If so, you should mention that.  -->
...
approximately 3 meters per pixel resolution [@planetlabs2022].
...
<!-- When introducing the various methods, you should say something about number of pixels for each–make it clear that the “point method” is a single pixel, the others are larger, depending on tree size. That’s important for thinking about the results downstream.  -->
...
`ForestTools` canopy delineation algorithm to create canopy polygons for
NDVI analysis.
...
<!-- Just curious–what drove the 30m buffer size? It seems to have created some weird artifacts (like the long canopy extension of the 2nd from left tree) and it seems extremely large to me–I think the largest canopies we measured last summer were 12-13m radius. Do you think changing that buffer to 15m would give better results? -->
