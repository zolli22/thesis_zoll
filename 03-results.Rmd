...
```{r final-model-small, echo = F, out.width = "90%"}
gg_point + gg_radius + gg_lidar +
  plot_layout(ncol = 2)
```

```{r species-health-fig, echo = F, out.width = "90%"}
ggplot(test_data_lidar, aes(fill = species, x = health_rat))+
  geom_bar()+
  scale_fill_manual(values = species_pal)+
  labs(x = "Health rating", y = "Count")
```
