...
<!-- Page 29: How much better did the polynomial function represent the data than the linear model? Adding parameters to the model usually improves the fit but you should also use the simplest model possible. Plus, the polynomial gives you the weird decrease in height at high DBH. Depending on how much of an improvement it is over the linear model, this tradeoff may not be worth it. I don’t think it would change much, since both models probably predict similar values for most of your data range. Also, why did you put all the species in a single model? Is that why the model is not a good fit for two of your species? -->
...
p_height_model <- my_park %>% 
  ...
  labs(subtitle = "Measurements from park trees data", y = "Tree Height Measurement", title = "Model")+
  guides(color = "none")
...
p_width_model <- my_park %>% 
  ...
  scale_color_manual(values = species_pal)+
  geom_smooth(method = "lm", se = F, formula = y ~ poly(x, degree = 2, raw = T))+
  labs(subtitle = "Measurements from park trees data", y = "Canopy Width Measurement", title = "Model")+
  guides(color = "none")
...
s_height_preds <- my_street_2 %>% 
  ...
  geom_point(alpha = .3)+
  scale_color_manual(values = species_pal)+
  labs(subtitle = "Predictions for street trees data", y = "Tree Height Prediction", title = "Predictions")

s_width_preds <- my_street_2 %>% 
  ggplot(aes(x = sqrt(DBH), y = width_preds, color = Species)) +
  geom_point(alpha = .3)+
  scale_color_manual(values = species_pal)+
  labs(subtitle = "Predictions for street trees data", y = "Canopy Width Prediction", title = "Predictions")
```
...
**fair** tree was most likely a conifer (Table
...
PSME, and THPL were all most likely to be rated as **fair**, but ACPL
was most likely to be rated as **good**. Within health category, a
**poor** tree was most likely an ACMA individual, a **fair** tree was
equally likely to be a PSME or THPL individual, and a **good** tree was
most likely an ACPL individual (Table \@ref(tab:table-freq-test-2)).

```{r table-freq-test-1, echo = F, results = 'asis'}
kable(list(tree_type_table, (tree_type_table/112)), 
      booktabs = TRUE, longtable = TRUE,
      caption = 'Frequency and probability tables of functional tree type and health rating for CNH trees. There is a statistically significant relationship between the two variables ($X^2$(2, N=112) = 10.184, p = 0.00615)', 
      caption.short = 'Frequency and probability tables of functional tree type and health rating for CNH trees'
)
```

```{r table-freq-test-2, echo = F, results = 'asis'}
kable(list(tree_species_table, (tree_species_table/112)), 
      booktabs = TRUE, longtable = TRUE,
      caption = 'Frequency and probability tables of functional tree type and health rating for CNH trees. There is a statistically significant relationship between the two variables ($X^2$(2, N=112) = 10.184, p = 0.00615)', 
      caption.short = 'Frequency and probability tables of functional tree type and health rating for CNH trees'
)
...
LiDAR trees is higher than for **fair** trees.
...
<!-- Page 37: Can you defend your decision to train and test the model on the same dataset? You say you did this due to sample size and imbalance. Is there a citation you can use to support this decision under these constraints? -->
...
an interaction term (health rating \~ lLiDAR NDVI \* species) was the
...
probability_data <- probs_point_species %>%
  mutate(method = "health rating ~ point NDVI * species") %>%
  bind_rows(probs_radius_species %>%
  mutate(method = "health rating ~ radius NDVI * species")) %>%
  bind_rows(probs_lidar_species %>%
  mutate(method = "health rating ~ LiDAR NDVI * species")) %>%
  mutate(method = fct_relevel(method, levels = c("point", "radius", "lidar")))
